...
function renderTimeline() {
  const recent = (state.data.events || []).slice(-10);
  $("timeline").innerHTML = recent.map((event) => `<div class="tape-event kind-${event.kind}"><span>R${event.round}</span><div><b>${event.title}</b><small>${event.detail}</small></div></div>`).join("");
...
function renderControls() {
  ...
  document.querySelectorAll(".layer-btn").forEach((button) => {
    const metadata = state.data.environment_layers?.[button.dataset.layer];
    button.disabled = metadata?.available === false;
    button.title = metadata?.available === false
      ? (metadata.provenance || "Layer unavailable in this incident")
      : "";
  });
  ...
  deploy.querySelector("small").textContent = done
    ? "Evaluator reveal available"
    : "Execute survey → Bayes update → replan";
...
async function resetCase(caseId = null) {
  try {
    ...
    state.mapZoom = 0;
    showTransition("NEW INCIDENT", "Initializing coastal response", "Loading real monitoring sites and a blinded synthetic hidden extent.");
...
async function deploy() {
  ...
  try {
    setBusy(true);
    showTransition("FIELD TEAM ACTIVE", `Surveying Site ${state.selectedSite}`, `Effort ${state.selectedEffort}. Hidden truth remains locked.`);
    const data = await api("/api/deploy", "POST", { site_id: state.selectedSite, effort: state.selectedEffort });
    ...
    if (obs) {
      ...
      $("transition-detail").textContent = `Effort ${obs.effort} · local belief ${fmtPct(obs.belief_before)} → ${fmtPct(obs.belief_after)}. The full posterior is recomputed before the next mission.`;
    }
    await new Promise((resolve) => setTimeout(resolve, 420));
    state.selectedSite = data.global_recommendations?.[0]?.site_id || state.selectedSite;
    if (state.selectedSite) state.selectedEffort = data.global_recommendations?.[0]?.recommended_effort || state.selectedEffort;
    render(data, before);
    setTimeout(hideTransition, 620);
  } catch (error) {
    hideTransition();
    alert(error.message);
  } finally {
...
async function reveal() {
  ...
  try {
    setBusy(true);
    showTransition("EVALUATE", "Revealing the hidden extent", "Scoring Marine, Static Response, and the interactive path on the same blinded incident.");
    const before = state.data;
    ...
    $("transition-detail").textContent = "Hidden occupancy remained unavailable during every recommendation and field decision.";
    setTimeout(hideTransition, 1050);
  } catch (error) {
    hideTransition();
    alert(error.message);
  } finally {
...
$("follow-marine-btn").addEventListener("click", () => {
  ...
  if (!top) return;
  state.selectedSite = top.site_id;
  state.selectedEffort = top.recommended_effort || state.selectedEffort;
  render(state.data);
});
...
document.querySelectorAll(".effort-btn").forEach((button) => button.addEventListener("click", () => {
  const effort = Number(button.dataset.effort);
...
$("zoom-in").addEventListener("click", () => {
  state.mapZoom = Math.min(5, state.mapZoom + 1);
...
$("graph").addEventListener("wheel", (event) => {
  ...
  state.mapZoom = Math.max(-1, Math.min(5, state.mapZoom + (event.deltaY < 0 ? 1 : -1)));
  renderMap(state.data);
}, { passive: false });
